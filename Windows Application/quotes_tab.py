...
class QuotesTab(customtkinter.CTkFrame):
    ...
    def load_textbox_file(self):
        with open(file=resource_path("quote_packs\\custom.txt"), encoding="utf-8", mode="r") as file:
            return file.read()

    def update_textbox(self, *args):
        with open(file=resource_path("quote_packs\\custom.txt"), encoding="utf-8", mode="w") as file:
